...
# Algorithm and code credit: https://github.com/masouduut94/MCTS-agent-python.git  
...
class StudentAgent(Agent):
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        """
        Args:
            chess_board (numpy array): records barriers and boarders for each block on the board, shape (x_max, y_max, 4)
            my_pos (tuple): (x, y)
            adv_pos (tuple): (x, y)
            max_step (int):

        Returns:
            best_action (tuple): ((x, y), dir)
                                    where (x, y) is the next position of your agent 
                                    and dir is the direction of the wall you want to put on.
        """
        start_time = time()
        ...
        if self.root is None:
            ...
            self.search(start_time + 30 - time() - buffer_time)
        else:
    ...
    def search(self, time_budget) -> None:
        ...
        Args:
            time_budget (int): alloted time in seconds
        """
        start_time = time()
    ...
    def expand(parent, state) -> bool:
        ...
        for action in state.actions():
            children.append(UCB1TunedNode(action, parent))
    ...
    def backup(node, turn: int, outcome: int) -> None:
        ...
        Args:
            node (UCB1TunedNode):
            turn (int): winner turn
            outcome (int): outcome of the rollout
        """
        # The reward is calculated for player who just played
    ...
    def best_action(self) -> tuple:
        ...
        Returns:
            action (tuple): ((x, y), dir); best action in terms of the most simulations number, unless the game is over
        """
        # Should never happen
...
class UCB1TunedNode:
    ...
    Args:
        action (tuple): ((x, y), dir), action applied to reach this node in the tree from parent node
        parent (UCB1TunedNode)
        N (int): times this position was visited
        Q (int): average reward (wins) from this position
        children (dict): dictionary of successive nodes
        outcome (int): if node is a leaf then outcome indicates the winner, else None
    """
    ...
    def value(self, explore: float = 0.5):
        ...
        Args:
            explore (float): constant that specifies how much the value should favor nodes 
...
class GameState:
    ...
    # Agent (me) is always player 1, adversay is always player 2.
    ...
    def determine_winner(self, my_score, adv_score) -> None:
        """
        Determine the winner based on the scores.

        Args:
            my_score (int): Player 1's score
            adv_score (int): Player 2's score
        """
        # I just played
    ...
    def check_endgame(self):
        ...
        Returns:
            ...
            player_1_score (int): the score of player 1
            player_2_score (int): the score of player 2
        """
        # Union-Find
        father = dict()
    ...
    def set_barrier(self, r, c, dir) -> None:
        """
        Set barrier on chess board (in both directions)
...
def tuple_equal(t1: tuple, t2: tuple) -> bool:
    """
