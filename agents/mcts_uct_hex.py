...
@register_agent("mcts_uct_hex_agent")
class MCTS_UCT_HexAgent(Agent):
    def __init__(self):
        super(MCTS_UCT_HexAgent, self).__init__()
        self.name = "MCTS_UCT_Hex_Agent"
        self.autoplay = True
    ...
    def expand(parent, state) -> bool:
        ...
        for action in state.actions():
            children.append(UCT_MctsNode(action, parent))
...
class UCT_MctsNode:
    ...
    def __init__(self, action: tuple = None, parent: object = None):
        """
        Initialize a new node with optional action and parent and initially empty
...
class GameState:
    ...
    # def would_lose(self, action: tuple) -> bool:
    #  """
    # Return True if the action indicated would lose the game,
    # False otherwise.
    # """
